...
function TieDecision({ divisionId, organizationId, standings, teams }: { divisionId: string; organizationId: string; standings: CompetitionWorkspace["standings"]; teams: Team[] }) {
  const unresolved = standings.filter((row) => row.rank === null)
  ...
  return <Card className="border-amber-500/40"><CardHeader><CardTitle>Unresolved standings tie</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">Confirm the published order and explain the league decision. Qualification remains paused until this is resolved.</p><ol className="divide-y rounded-md border">{ordered.map((teamId, index) => <li key={teamId} className="flex items-center gap-2 px-3 py-2"><span className="flex-1 text-sm font-medium">{teams.find((team) => team.id === teamId)?.name ?? "Team"}</span><Button size="icon-sm" variant="ghost" disabled={index === 0} onClick={() => move(index, -1)}><ArrowUp /></Button><Button size="icon-sm" variant="ghost" disabled={index === ordered.length - 1} onClick={() => move(index, 1)}><ArrowDown /></Button></li>)}</ol><Textarea aria-label="Tie decision reason" placeholder="Explain how the league confirmed this order." value={reason} onChange={(event) => setReason(event.target.value)} /><Button disabled={reason.trim().length < 10 || mutation.isPending} onClick={async () => { try { await mutation.mutateAsync({ orderedTeamIds: ordered, poolId, reason, teamIds: unresolved.map((row) => row.team_id) }); toast.success("Tie decision published") } catch (error) { toast.error(getApiErrorMessage(error)) } }}>Publish decision</Button></CardContent></Card>
}
